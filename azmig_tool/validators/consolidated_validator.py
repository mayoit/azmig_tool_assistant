...
class ConsolidatedValidator:
    ...
    def validate_all(self, configs: List[ConsolidatedMigrationConfig]) -> Dict[str, Any]:
        ...
        print(
            f"[blue]Step 1: Validating {len(lz_configs)} unique Landing Zone configurations[/blue]")
        ...
        for config in configs:
            ...
            if lz_result and (lz_result.is_ready() or
                ...
                server_results = self.server_validator.validate_all(
                    [server_config])

                # Get the validation results for this machine
                machine_validations = server_results.get(
                    server_config.machine_name, [])

                # Create a machine validation report
                overall_status = "PASSED" if all(
                    v.passed for v in machine_validations) else "FAILED"
